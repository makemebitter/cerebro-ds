...
class ResNet50(object):
    def __init__(self, model_input, num_classes=1000, model_name='resnet50'):
        self.model_input = model_input
        self.model_name = model_name
        self.retrain_layers = True
    ...
    def __calc_conv2(self):
        ...
        self.conv2_2 = tf.nn.relu(self.__identity_block(input_layer=self.conv2_1, name='2b',
                                                        num_filters=64))
        self.conv2_3 = tf.nn.relu(self.__identity_block(input_layer=self.conv2_2, name='2c',
                                                        num_filters=64))

    def __calc_conv3(self):
        self.conv3_1 = tf.nn.relu(self.__conv_block(input_layer=self.conv2_3, name='3a',
                                                    num_filters=128))
        self.conv3_2 = tf.nn.relu(self.__identity_block(input_layer=self.conv3_1, name='3b',
                                                        num_filters=128))
        self.conv3_3 = tf.nn.relu(self.__identity_block(input_layer=self.conv3_2, name='3c',
                                                        num_filters=128))
        self.conv3_4 = tf.nn.relu(self.__identity_block(input_layer=self.conv3_3, name='3d',
                                                        num_filters=128))

    def __calc_conv4(self):
        self.conv4_1 = tf.nn.relu(self.__conv_block(input_layer=self.conv3_4, name='4a',
                                                    num_filters=256))
        self.conv4_2 = tf.nn.relu(self.__identity_block(input_layer=self.conv4_1, name='4b',
                                                        num_filters=256))
        self.conv4_3 = tf.nn.relu(self.__identity_block(input_layer=self.conv4_2, name='4c',
                                                        num_filters=256))
        self.conv4_4 = tf.nn.relu(self.__identity_block(input_layer=self.conv4_3, name='4d',
                                                        num_filters=256))
        self.conv4_5 = tf.nn.relu(self.__identity_block(input_layer=self.conv4_4, name='4e',
                                                        num_filters=256))
        self.conv4_6 = tf.nn.relu(self.__identity_block(input_layer=self.conv4_5, name='4f',
                                                        num_filters=256))

    def __calc_conv5(self):
        self.conv5_1 = tf.nn.relu(self.__conv_block(input_layer=self.conv4_6, name='5a',
                                                    num_filters=512))
        self.conv5_2 = tf.nn.relu(self.__identity_block(input_layer=self.conv5_1, name='5b',
                                                        num_filters=512))
        self.conv5_3 = tf.nn.relu(self.__identity_block(input_layer=self.conv5_2, name='5c',
                                                        num_filters=512))
